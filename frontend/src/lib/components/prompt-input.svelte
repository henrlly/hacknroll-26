        ...
        <Card.Content>
            <form class="flex flex-col items-start" onsubmit={handleSubmit}>
                <Textarea
                        ...
                        name="prompt" placeholder="Enter video idea" bind:value={prompt}/>

                <input type="hidden" name="voice" bind:value={voice}>

                <div class="w-full flex justify-between">
                    <div class="flex gap-2">
                        <Badge class="h-6 cursor-pointer" variant={voice === "trump" ? "default" : "outline"}
                               onclick={() => { voice = "trump" }}>Trump
                        </Badge>
                        <Badge class="h-6 cursor-pointer" variant={voice === "obama" ? "default" : "outline"}
                               onclick={() => { voice = "obama" }}>Obama
                        </Badge>
                        <!-- <Badge class="h-6" variant={voice === "peter" ? "default" : "outline"} onclick={() => { voice = "peter" }}>Peter</Badge> -->
                    </div>
                    <button class="w-20 {buttonVariants({ variant: "default" })}">Submit
                    </button>
                </div>
            </form>
        </Card.Content>
